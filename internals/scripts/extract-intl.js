  ...
  try {
    ...
    for (const message of result['react-intl'].messages) {
      for (const locale of locales) {
        ...
        localeMappings[locale][message.id] = (oldLocaleMapping) ? oldLocaleMapping : newMsg // eslint-disable-line no-unneeded-ternary
      }
    }
  } catch (error) {
...
(async function main () {
  ...
  mkdir('-p', 'app/translations') // eslint-disable-line no-undef
  for (const locale of locales) {
    const translationFileName = `app/translations/${locale}.json`
    ...
    try {
      ...
      localeTaskDone( // eslint-disable-line no-undef
